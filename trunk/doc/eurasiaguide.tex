...
\lineii{ httpfile.path_info      }{ 参见 cgi1.1 协议关于 SCRIPT_NAME 与 PATH_INFO 的说明, 如果 uri 是 "/show?page=10" 那么 path_info 为 "/show" }
\lineii{ httpfile.script_name    }{ 参见 cgi1.1 协议关于 SCRIPT_NAME 与 PATH_INFO 的说明 }
...
\note{修改 httpfile 对象的 request_uri、path_info、script_name、query_string
这四个属性是联动的, 他们也会同时影响到 httpfile.environ 中的
REQUEST_URI、PATH_INFO、SCRIPT_NAME、QUERY_STRING 这三个环境变量 (但 httpfile.uri 不受影响)。
